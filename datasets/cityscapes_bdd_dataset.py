...
class CityscapesxBDDDataset(torch.utils.data.Dataset):
    """
    Dataset which generates pairs of labelled and unlabelled data from the Cityscapes and BDD datasets.

    Returns:
        - labelled_dict: dict containing the labelled image and label
        - bdd_dict: dict containing two unlabelled images with different appearance
    """
    ...
    def __getitem__(self, index):
        ...
        # normalizing
        img = normalize_img(img, imagenet=self.use_imagenet_norm)
        ...
        if not self.only_labelled:
            ...
            # normalizing
            bdd_img_1 = normalize_img(bdd_img_1, imagenet=self.use_imagenet_norm)
            bdd_img_2 = normalize_img(bdd_img_2, imagenet=self.use_imagenet_norm)
