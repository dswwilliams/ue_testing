...
class GammaSSLModel(nn.Module):
    ...
    def get_val_seg_masks(self, imgs):
        ...
        if self.opt.use_proto_seg:
            ...
            seg_masks_K_q = self.seg_net.get_query_seg_masks(imgs, include_void=False, high_res=True)
        ...
        if self.opt.frozen_target:
            seg_masks_K_t = self.target_seg_net.get_target_seg_masks(imgs, include_void=False, high_res=True)
        else:
            seg_masks_K_t = self.seg_net.get_target_seg_masks(imgs, include_void=False, high_res=True)
