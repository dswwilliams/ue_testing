...
def calculate_ue_metrics(
    ...
    """
    Calculates quality of misclassification detection for a given set of segmentations and uncertainty maps.
    Specifically, it calculates the following raw metrics:
        - n_inaccurate_and_certain
        - n_accurate_and_certain
        - n_uncertain_and_accurate
        - n_uncertain_and_inaccurate
    which are processed into uncertainty estimation metrics.
    """

    bs = segmentations.shape[0]
    device = segmentations.device

    # defining thresholds in range: [0, max_uncertainty]
    ...
    # clip uncertainty maps to max_uncertainty
    uncertainty_maps = torch.clip(uncertainty_maps, max=max_uncertainty)
...
def calculate_p_certain(metrics_totals, states):
    """ Calculates proportion of predictions that are certain. """
    tp = metrics_totals[states["tp"]]
    tn = metrics_totals[states["tn"]]
    fp = metrics_totals[states["fp"]]
    fn = metrics_totals[states["fn"]]
...
def calculate_p_accurate(metrics_totals, states):
    """
    Calculates proportion of uncertainty predictions that are accurate.
    NOTE: This is not the same as the segmentation accuracy.
    """
    tp = metrics_totals[states["tp"]]
    tn = metrics_totals[states["tn"]]
    fp = metrics_totals[states["fp"]]
...
def calculate_mean_stats(metrics_totals):
    """ Calculates mean statistics from raw metrics. """
...
def calculate_metrics_suite(val_metrics_totals, val_metrics_counts, states):
    """ Calculates a suite of metrics from raw metrics. """
    processed_metrics = {}
...
def plot_ue_metrics(processed_metrics, validation_count, dataset_name, plot_plots=False, vis=None):
    """ Plots uncertainty estimation metrics to wandb or visdom. """
    if vis is None:
        USE_WANDB = True
        import wandb
    else:
...
def update_running_totals(val_metrics_totals, val_metrics_counts, val_metrics_dict):
    """ Updates running totals (val_metrics_totals) and counts (val_metrics_counts) from val_metrics_dict. """
...
def init_val_ue_metrics(num_thresholds):
    """ Initializes running totals and counts for uncertainty estimation metrics. """
...
def validate_batch(
    ...
    """ Calculates processed uncertainty estimation metrics for a given batch of validation data and model. """
    ...
    for output_name, output in outputs.items():
        ...
        ue_metrics[output_name]["miou"] = calculate_miou(
                                                segmentations=output["segs"], 
                                                labels=val_labels, 
                                                num_classes=model.num_known_classes,
                                                )
    return ue_metrics

@torch.no_grad()
def get_max_uncertainty(
            val_imgs, 
            model,
            ):
    """ Finds the maximum uncertainty value in a batch of validation imgs. """
    
    max_uncertainty = 0

    outputs = model.get_val_seg_masks(val_imgs)
    for output in outputs.values():
        max_uncertainty = max(output["uncertainty_maps"].max(), max_uncertainty)
