...
def normalize_img(img, imagenet=True):
    """
    Normalizes an image based on ImageNet statistics or to [-1, 1].
    """
    ...
    # if img is pytorch tensor
    elif isinstance(img, torch.Tensor):
        img = img.float()
        if (img > 2).any():
            img = img/255.0
        if imagenet:
            img = img - torch.tensor([0.485, 0.456, 0.406]).unsqueeze(1).unsqueeze(2).float()
            img = img / torch.tensor([0.229, 0.224, 0.225]).unsqueeze(1).unsqueeze(2).float()
        else:
            # [0,1] -> [-1, 1]
            img = (img - 0.5) * 2
    return img
